...
const SCENARIO_DESCRIPTIONS = {
  auto:              '正在自动巡演所有感知场景。',
  empty_room:        '空房间基线校准，房间内无人。',
  single_breathing:  '通过 WiFi 信号微动检测生命体征。',
  two_walking:       '通过 CSI 多路复用同时追踪多人。',
  fall_event:        '基于加速度特征分析的突发姿态变化检测。',
  sleep_monitoring:  '睡眠期间呼吸模式与呼吸暂停事件监测。',
  intrusion_detect:  '被动周界监控——无摄像头，纯射频感知。',
  gesture_control:   '基于 DTW 的手势识别，从手臂运动签名识别。',
  crowd_occupancy:   '从聚合 CSI 方差估计房间内人员数量。',
  search_rescue:     'WiFi-MAT 多基地模式的穿墙幸存者检测。',
  elderly_care:      '持续步态分析，提早发现行动力下降。',
  fitness_tracking:  '从身体运动学计数次数并分类训练动作。',
  security_patrol:   '多区域在场巡查，免摄像头运动热力图。',
};
...
// Chinese display labels for module badges (key stays for color lookup)
const MODULE_LABELS = {
  VITALS:    '生命体征',
  GAIT:      '步态',
  FALL:      '跌倒',
...
export class HudController {
  ...
  updateHUD(data, demoData) {
    ...
    // FIX: clamp 到 [0,100] — sensing-server 偶发返回 confidence>1.0 (1.43 等) 导致显示 143%
    const targetConf = Math.min(100, Math.max(0, Math.round((cls.confidence || 0) * 100)));

    // Smooth lerp transitions (blend 4% per frame toward target — very stable)
    // FIX: target=0 时若 presence=true 则保持上次值（避免 ESP32 单节点 vitals 偶发 0 导致数字闪烁）；
    //      presence=false 才平滑归零
    const lerpFactor = 0.04;
    const decayFactor = 0.02;
    const hasPerson = cls.presence === true;
    this._lerpHr = targetHr > 0 ? lerp(this._lerpHr, targetHr, lerpFactor)
                                : hasPerson ? this._lerpHr
                                            : lerp(this._lerpHr, 0, decayFactor);
    this._lerpBr = targetBr > 0 ? lerp(this._lerpBr, targetBr, lerpFactor)
                                : hasPerson ? this._lerpBr
                                            : lerp(this._lerpBr, 0, decayFactor);
    this._lerpConf = targetConf > 0 ? lerp(this._lerpConf, targetConf, lerpFactor)
                                    : hasPerson ? this._lerpConf
                                                : lerp(this._lerpConf, 0, decayFactor);
    ...
    if (presEl) {
      ...
      if (ml === 'active') { presEl.classList.add('presence--active'); presLabel.textContent = '活动中'; }
