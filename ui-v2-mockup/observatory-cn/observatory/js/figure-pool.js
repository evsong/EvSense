...
const JOINT_SIGMA = [
  0.06,  // 0 nose
  0.03, 0.03, 0.04, 0.04,        // eyes, ears
  0.10, 0.10,                     // shoulders
  0.07, 0.07, 0.05, 0.05,         // elbows, wrists
  0.10, 0.10,                     // hips
...
/** Build a soft round sprite for Three.js Points so each particle looks like a glow disc. */
function _makeParticleSprite() {
  const size = 64;
...
const _PARTICLE_SPRITE = (typeof document !== 'undefined') ? _makeParticleSprite() : null;
...
export class FigurePool {
  ...
  _updateParticles(fig, breathPulse, hrPulse) {
    ...
    for (const bone of fig.bones) {
      ...
      for (let p = 0; p < PARTICLES_PER_BONE; p++) {
        ...
        arr[w++] = a.y + (b.y - a.y) * t + _gauss() * jitter;
        arr[w++] = a.z + (b.z - a.z) * t + _gauss() * jitter;
      }
    }
    fig.particles.geometry.attributes.position.needsUpdate = true;
    // Particle opacity tracks the glow setting + breath pulse for liveness
    fig.particleMat.opacity = Math.min(0.95,
      0.55 + this._settings.glow * 0.2 + Math.abs(breathPulse) * 1.5);
  }
  ...
  applyKeypoints(fig, kps, breathPulse, pos, elapsed = 0, pose = 'standing', vitals = null, hrPulse = 0) {
    ...
    // Body segments — keep their position update so colors/shapes stay correct,
    // but render them nearly transparent (0.03) so the particle cloud dominates
    // visually instead of the old solid green capsule.
    for (const seg of fig.bodySegments) {
      ...
      seg.mat.emissiveIntensity = 0.05 + Math.abs(breathPulse) * 0.2;
    }
  ...
  applyColors(wireColor, jointColor) {
    for (const fig of this._figures) {
      ...
      if (fig.particleMat) fig.particleMat.color.copy(wireColor);
    }
  }
}
